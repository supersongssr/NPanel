...
- [x] router : /services/ 页面 在用户购买套餐的时候, 如果用户已经有套餐了,就应该提醒用户, 购买多个套餐不会叠加,是同时生效的, 点击同意才能继续购买 .
  - 当前现状:
    有用户反馈, 套餐快到期了,想再购买套餐续期,但是发现没有续费,时间叠加了.
  - 请您修改为:
    用户购买套餐的时候, 会显示用户已有的套餐,如果用户已有套餐剩余有效期 > 7天,就这样提醒用户,套餐还剩余有效期. 套餐会同时生效不会叠加, 同意再继续购买.
  - 验收方式:
