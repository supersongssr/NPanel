        ...
        foreach ($nodeList as $key => $node) {
            ...
            if ( $node->type == 2 && ($vmess_sub || $ver == "2" || $v2ray_sub || $rocket_sub) ) {       // 获取 vmess节点   
                ...
                if (!empty($node->v2_ech)) {
                    $scheme .= '&ech=' . urlencode($node->v2_ech);
                }
                if ($isReality) {
    ...
    private function generateSingboxConfig($nodeList, $user)
    {
        ...
        foreach ($nodeList as $node) {
            ...
            if ($node->type == 2) {
                ...
                // 添加 TLS 配置
                if ($tlsEnabled) {
                    // client-fingerprint: 以节点 v2_fp 为准 (register 阶段写入).
                    // reality / xhttp-cdn-hy2 强制 firefox (uTLS = 抗 GFW 主动探测核心伪装层);
                    // 其余节点沿用 ios.
                    $clientFp = $node->v2_fp ?: 'ios';
                    $outbound['tls'] = [
                        'enabled' => true,
                        'server_name' => $node->v2_sni,
                        'utls' => [
        ...
        foreach ($nodeList as $node) {
            ...
            elseif ($node->type == 3) {
                ...
                if ($tlsEnabled) {
                    ...
                    // client-fingerprint: 以节点 v2_fp 为准 (register 阶段写入).
                    // reality / xhttp-cdn-hy2 强制 firefox (uTLS 指纹 = 抗 GFW 主动探测核心伪装层);
                    ...
                    // xhttp-cdn / xhttp-cdn-hy2 (CF CDN): 启用 ECH 隐藏真实 SNI, 抗域名封锁/干扰.
                    // 仅 xhttp 走 CF 的节点有 v2_ech (hy2 不经过 CF, v2_ech 为空, 不输出此块).
                    // record=true: Mihomo 从 server_name 的 DNS HTTPS 记录自动拉取 ECHConfigList
                    // (Cloudflare 在该记录里发布 ECH 公钥).
                    if (!empty($node->v2_ech)) {
                        $yaml .= "    ech-opts:\n";
                        $yaml .= "      enable: true\n";
                        $yaml .= "      record: true\n";
